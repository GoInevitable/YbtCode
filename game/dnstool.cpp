 ...
 *
 * High-level design:
 * - Construct and send simple UDP DNS queries (A record) to listed DNS servers.
 * - Measure round-trip time (RTT) per query, collect min/avg/max and loss%.
 * - Parse returned A/AAAA answers and attempt a light TCP connect to each returned IPv4
 *   address to check basic reachability.
 * - Provide both interactive CLI and command-line modes; supports saving defaults.
 *
 * Notes:
 * - This file prefers lightweight hand-rolled DNS parsing for portability and zero-deps.
 * - On Windows, Winsock and Iphlpapi are used to enumerate system DNS servers.
 */
...
static void save_config(const string &dir, const vector<string>& domains, const vector<string>& servers, int probes, int timeout_ms, bool include_ipv6) {
	// Persist a minimal key=value config to `dnstool.conf` in exe directory.
...
static void load_config(const string &dir, vector<string>& domains, vector<string>& servers, int &probes, int &timeout_ms, bool &include_ipv6) {
	// Load previously saved config if present; tolerant to parse errors.
	string path = dir.empty()?"dnstool.conf": dir + "dnstool.conf";
	ifstream f(path);
	if(!f) return;
	string line;
...
vector<uint8_t> build_dns_query(const string &hostname, uint16_t id) {
	// Build a standard DNS query packet (header + single QNAME for Type A)
	vector<uint8_t> buf;
	buf.resize(12);
	// Header
	buf[0] = (id >> 8) & 0xFF;
...
vector<string> get_system_dns_servers() {
	// Enumerate system-configured DNS servers. On Windows uses GetAdaptersAddresses,
	// on POSIX reads /etc/resolv.conf. Returns deduplicated list of IP strings.
	vector<string> res;
#if defined(_WIN32) || defined(_WIN64)
...
double probe_once(const string &server_ip, const string &hostname, int timeout_ms) {
	// Send a single UDP DNS query and return RTT in milliseconds or -1.0 on failure.
...
int main(int argc, char** argv) {
	...
	// Prepend system DNS servers (filtered for IPv6 by default) so user's
	// configured servers are tested after system DNS entries.
	...
	// helper: send query, parse A/AAAA answers and return measured RTT.
	// Also populates out_ips with any returned addresses.
	...
	auto tcp_connect_timeout = [&](const string &ip, int port, int timeout_ms) -> bool {
		// Only attempts IPv4 TCP connect. Uses non-blocking connect + select
		// to implement a per-connection timeout without creating threads.
	...
	// Open log file in executable directory (append mode). If unavailable,
	// the program continues but without file logging.
